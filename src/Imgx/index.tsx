...
const ImgxHook = ({
  ...
  const handleImgUrl = async () => {
    ...
    let newUrlStr
    if (imageLoadType === 'oss') {
      newUrlStr = getImgOssGzip({ src, width: imgHitWidth, quality, iswebp })
    } else {
      newUrlStr = getImgGzip({ src, width: imgHitWidth, quality, iswebp });
    }
    setImgUrl(newUrlStr);
  };
  ...
  const handlePlaceholderSrc = () => {
    ...
    const newImgType = {
      ...
      oss: `${curSrc}?x-oss-process=image/resize,w_100`,
